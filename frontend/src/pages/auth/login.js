	...
	const handleSubmit = async (values, { setSubmitting, setFieldError, setFieldValue }) => {
		...
		await fetch("/api/v1/auth/login", {
			...
			.catch((error) => {
				...
				if (errorObject.message === "Email not verified")
					router.push(
						{
				...
				else if (errorObject.message === "Email not found") {
					setFieldError("Email", errorObject.message);
				} else if (errorObject.message === "Password incorrect") {
					setFieldError("Password", errorObject.message);
				} else setFieldValue("SubmissionError", errorObject.code + ": " + errorObject.message);
	...
	return (
		<>
			<Head>
				...
				<meta name="description" content="Login - ShopStop" />
			</Head>
			<Layout>
						...
						>
							{({ isSubmitting, values }) => (
								<Form className="mt-6 px-6 sm:px-24 py-6 sm:py-16 -mx-6 rounded-3xl bg-[#8D72E1]/70">
									...
									{values.SubmissionError && <div className="mt-2 text-red-500 font-medium text-sm">{values.SubmissionError}</div>}
								</Form>
							)}
						</Formik>
					</div>
