...
const auth_validation = require("../../validations/Admin/auth.validation");
...
module.exports = {
	register: async (req, res, next) => {
		try {
			...
			if (process.env.ADMIN_CODE !== JoiResult.AdminCode) throw createError.Unauthorized("Admin code invalid");

			bcrypt.hash(JoiResult.Password, 10, async (err, hash) => {
				await axios.post(`${process.env.DB_HOST}/users`, {
					id: uuidv4(),
