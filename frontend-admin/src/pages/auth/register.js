...
export default function Register({ from }) {
	...
	const handleSubmit = async (values, { setSubmitting, setFieldError, setFieldValue }) => {
		...
		await fetch(`/api/v1/public/Admin/auth/register`, {
			method: "POST",
			headers: {
				"Content-Type": "application/json",
			},
			body: JSON.stringify({ Email: values.Email, Password: values.Password, AdminCode: values.AdminCode }),
		})
			.then(async (response) => {
			...
			<Layout>
				<div className="min-h-full flex items-center justify-center">
					<div className="w-full max-w-xl">
						...
						<Formik
							initialValues={{ Email: "", Password: "", RePassword: "", AdminCode: "" }}
							...
							{({ isSubmitting, values }) => (
								<Form className="mt-6 px-6 sm:px-24 py-6 sm:py-16 -mx-6 rounded-3xl bg-[#8D72E1]/70">
									...
									<div className="mt-4 relative group">
										...
										<ErrorMessage name="AdminCode" component="div" className="mt-2 text-red-500 font-medium text-sm" />
									</div>
